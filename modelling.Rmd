...
<!-- PONER AQUÍ EL NÚMERO TOTAL DE EXPERIMENTOS: CUANTOS MODELOS SE HACEN DE CADA TIPO, CONTANDO PARAMETER FINE TUNING Y TEST PERFORMANCE REVISAR -->
...
<!-- HACER AQUI EL HEATMAP DE LOS ERRORES OBTENIDOS DE CROSSVALIDACION REVISAR -->
...
<!-- PONER LAS TABLAS CON LA MEDIA DE ACCURACY OBTENIDA ENTRE TODOS LOS ALISADOS. CREAR REGLA: SI COMO MINIMO 3 ALISADOS DICEN QUE SUBIRA, ES MAS ROBUSTO... REVISAR -->
...
```{r}
#
table(c(as.character(imp_KO[,2]),as.character(imp_AAPL[,2]),as.character(imp_AXP[,2]),as.character(imp_WFC[,2]))) %>% sort
```
...
round(table(c(as.character(imp_KO[,2]),as.character(imp_AAPL[,2]),as.character(imp_AXP[,2]),as.character(imp_WFC[,2])))/sum(table(c(as.character(imp_KO[,2]),as.character(imp_AAPL[,2]),as.character(imp_AXP[,2]),as.character(imp_WFC[,2]))))*100,2) %>% sort
```

\noindent
ii) Cuál ha sido la variable más importante en el *split* con la frecuencia relativa más alta de **entre todos los modelos para cada empresa**?
